...
- 어떻게 자바스크립트 코드를 읽어서 분석하는가?
  ...
  2.  **자바스크립트 코드를 분석할 수 있는 파서(parser)로 구조화한다.**
      ...
      - 위와 같은 자바스크립트 코드를 espree로 분석하면 아래와 같이 JSON 형태로 구조화된 결과를 얻을 수 있음
        ...
        ```
      - 단순히 변수인지, 함수인지 등만 파악하는 것이 아니라 코드의 정확한 위치와 같은 세세한 정보까지 분석
      - 이런 자세한 정보가 있어야만 ESLint나 Prettier같은 도구가 코드의 줄바꿈, 들여쓰기 등을 파악할 수 있음
      </details>
  ...
  3.  **2번에서 구조화한 트리를 AST(Abstract Syntax Tree)라 하고, 이 구조화된 트리를 기준으로 각종 규칙과 대조한다.**
      ...
      - debugger의 사용을 금지하는 규칙과 대조하는 예시
